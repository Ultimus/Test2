   ...
   public static void drawBalls() {
      ...
      for(int i = 0; i < balls.size(); i++) {
         ...
         try {   
            ...
            /*if (ball.targetX == tower.x && ball.targetY == tower.y){ // falls Boot zurückschiesst
         	   ball.setX((int) (ship.getX()-(ship.getX()- tower.getX())*ball.getFactor()));
         	   ball.setY((int) (ship.getY()-(ship.getY()- tower.getY())*ball.getFactor()));
         	   if (ball.targetX == ship.x  && ball.x == ship.x && ship.y == ball.y && ball.targetY == ship.y)
         		   {towers.remove(ball.towerID);}
            }*/
	...
	public static void killingTower (Enemy compareE, int k){
		...
		for (int i =0; i < towers.size(); i++){
			...
			if(Math.sqrt(dx*dx+dy*dy)<= 300){
				balls.add(new CanonBall(k, i, 0.1, compareE.getX(), compareE.getY(), compareT.getX(), compareT.getY()));
			}
		}
	}
		 ...
		 if(!towers.isEmpty()){
			 ...
			 for(int i = 0; i < towers.size(); i++){
				 ...
				 if(compareT.getTyp()==3 || compareT.getTyp()==6 || compareT.getTyp()==9){
						...
						StdDraw.picture(compareT.getX()+15,compareT.getY(), "Windmuehle2Schatten.png");
						StdDraw.picture(compareT.getX()+25,compareT.getY()+5, "Windmuehle1Schatten.png", compareT.getMillTurn());
						StdDraw.picture(compareT.getX(),compareT.getY(), "Windmuehle2.png");						
						if(compareT.getTyp()==9)	StdDraw.picture(compareT.getX()-5,compareT.getY()+5, "Windmuehle1.png", compareT.getMillTurn());
						if(compareT.getTyp()==3)	StdDraw.picture(compareT.getX()-5,compareT.getY()+5, "WindmuehleStufe1.png", compareT.getMillTurn());
						if(compareT.getTyp()==6)	StdDraw.picture(compareT.getX()-5,compareT.getY()+5, "WindmuehleStufe2.png", compareT.getMillTurn());
...
//////////////////Draw Route	/////////////////////////////////////////////////
	 ...
	 public static void drawInterface(){
		...
		StdDraw.textLeft(WIDTH-127, 15, "Wave   "+wavelevel);
		StdDraw.textLeft(WIDTH-127, 37, "Score  "+score);
	 }
