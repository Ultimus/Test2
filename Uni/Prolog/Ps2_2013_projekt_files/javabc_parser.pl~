...
parse1 :-
  parse('PowerNeq.byc').
parse2 :-
  parse('ModuloCalc.byc').
parse3 :-
  parse('PowerWithFun.byc').
parse4 :-
  parse('SimpleDFA.byc').
...
program --> labelled_instr(Nr,Inst,Size),!, 
	     {assert(instr(Nr,Inst,Size))},
...
labelled_instr(Nr,Inst,Size) -->  label(Nr),{assert(block(Nr,To, Condition)),check_jmp(Inst,To, Condition)},
...
check_jmp(Instr, To, Condition):-
	Instr = if(_,To),Condition = true,!;
	Instr = if1(_,_,To),Conditional = true,!;
	Instr = goto(To),Conditional = false,!.
%if the instruction is a jump instruction we save the labelnumber of the instr, we also have a flag to save if the instr jumps with or without a condition
...
instr(if1('!=',0,Label),3) --> "ifne", ws, labelnr(Label),!.	% error found here
...
instr(iop(+),1) --> "iadd",!.
instr(iop(-),1) --> "isub",!.
instr(iop(/),1) --> "idiv",!. %Division
instr(iop(**),1) --> "ipow",!. %Power!!!
